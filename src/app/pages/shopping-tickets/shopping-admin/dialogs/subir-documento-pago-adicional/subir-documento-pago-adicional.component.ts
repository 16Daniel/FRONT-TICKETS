...
@Component({
  selector: 'app-subir-documento-pago-adicional',
  standalone: true,
  imports: [CommonModule, DialogModule,FileUploadModule,ToastModule,ProgressBarModule],
  providers: [MessageService],
  templateUrl: './subir-documento-pago-adicional.component.html',
  styleUrl: './subir-documento-pago-adicional.component.scss',
})
export class SubirDocumentoPagoAdicionalComponent implements OnInit {
@ViewChild('fileUpload') fileUpload!: FileUpload;
@Output() closeEvent = new EventEmitter<boolean>();
@Input() Reg:PagoAdicional|undefined; 
...
@Input() tipoDoc:number = 1; 
  ...
  ngOnInit(): void { }
    ...
    async onUpload(event: any) {
      let URL = ""; 
      let archivos:string[] = [];
        let path = this.tipoDoc == 1 ? 'documentos_viaticos/' : 'documentos_bonos/'; 
     ...
     this.Reg!.documentos = JSON.stringify(archivos); 
  ...
  async actualizarUrl(item:PagoAdicional)
  {
