  ...
  this.subscriptiontk = this.apiserv
    ...
    .subscribe({
      next: (data) => {
        ...
        let temp1:TicketDB[] = this.arr_tickets.filter(x => x.prioridadsuc == 'PÁNICO');
        let temp2:TicketDB[] = this.arr_tickets.filter(x => x.prioridadsuc == 'ALTA');
        let temp3:TicketDB[] = this.arr_tickets.filter(x => x.prioridadsuc == 'MEDIA');
        let temp4:TicketDB[] = this.arr_tickets.filter(x => x.prioridadsuc == 'BAJA');
...
{
  let temp = this.arr_tickets.filter(x => x.prioridadsuc == value && x.statusSuc != 'PÁNICO'); 
  return temp.sort(
    (a, b) => b.fecha.toDate().getTime() - a.fecha.toDate().getTime()
  );
}
...
getusuarioshelp()
{
  this.apiserv.getusers().subscribe({
    next: data => {
       ...
       this.catusuarioshelp = this.catusuarioshelp.filter(x => x.idRol == '4');
     this.cdr.detectChanges();
    },
    error: error => {
       console.log(error);
...
filtrarT()
{
  ...
  if(this.filterPrioridad != undefined)
    ...
    if(this.filterarea != undefined)
      {
        this.arr_tickets = this.arr_tickets.filter(x => x.idproveedor == this.filterarea.id);
      }
  ...
  if(this.formtiposoporte != '')
    {
      ...
      this.apiserv
      .updateTicket(this.itemtk)
      .then(() => { this.showMessage('success',"Success","Enviado correctamente"); this.formtiposoporte = ''; })
      .catch((error) => console.error('Error al actualizar los comentarios:', error));
    }   
}
...
getResponsablesUC(ids:string):string
{
  ...
  for(let item of this.catusuarioshelp)
    {
      const existeSucursal = item.sucursales.some(sucursal => sucursal.id == ids);
      if(existeSucursal)
        {
