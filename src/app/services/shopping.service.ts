...
export class ShoppingService {
  ...
  getCompraById(idTicket: string): Observable<any> {
    const ticketDoc = doc(this.firestore, `compras_administracion/${idTicket}`);
    return docData(ticketDoc, { idField: 'id' }) as Observable<any>;
  }

  getPagoById(idTicket: string): Observable<any> {
    const ticketDoc = doc(this.firestore, `pagos_adicionales_administracion/${idTicket}`);
    return docData(ticketDoc, { idField: 'id' }) as Observable<any>;
  }
