...
import { itable } from '../../interfaces/Checadas';
import { CommonModule, DatePipe } from '@angular/common';
import { NgxChartsModule } from '@swimlane/ngx-charts';

@Component({
  selector: 'app-grafica-horas-trabajo',
  standalone: true,
  imports: [CommonModule,NgxChartsModule],
  providers:[DatePipe],
  templateUrl: './grafica-horas-trabajo.html',
  styleUrls: ['./grafica-horas-trabajo.scss'],
})
export class GraficaHorasTrabajo implements OnInit {
@Input() data:itable[] = [];
...
constructor(private datePipe: DatePipe, public cdr:ChangeDetectorRef)
{
  ...
  ngOnInit(): void 
  { 
      let fecha = new Date(this.fechaIni);
      while(fecha<this.fechaFin)
        {
          let dia = this.datePipe.transform(fecha, 'dd-MM-yyyy')!;
          let horasTrabajo = 0; 
          let dataDia = this.data.filter(x=> x.fecha.getDate() == fecha.getDate() && x.fecha.getMonth() == fecha.getMonth() && x.fecha.getFullYear() == fecha.getFullYear()); 
          for(let itemd of dataDia)
            {
              horasTrabajo = horasTrabajo + itemd.estadia; 
            }
          this.single.push({name:dia,value:horasTrabajo});
