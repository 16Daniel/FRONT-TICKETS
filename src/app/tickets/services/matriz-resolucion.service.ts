...
export class MatrizResolucionService {
  // Nota: Mantenemos 'cat_matriz_atencion' para preservar intacta la colección existente en Firebase
  private readonly nombreColeccion = 'cat_matriz_atencion';
  private cacheObservables = new Map<string, Observable<MatrizResolucion>>();

  constructor(private firestore: Firestore) {}

  /**
   * Genera la matriz de resolución predeterminada 3×3 con los tiempos SLA base:
   * [ 2 d ]  [ 12 h ]  [ 2 h ]
   * [ 4 d ]  [ 1 d ]   [ 8 h ]
   * [ 5 d ]  [ 3 d ]   [ 1.5 d ]
   */
  obtenerMatrizPredeterminada(idArea: string, nombreArea: string = ''): MatrizResolucion {
    const celdasBase: CeldaMatrizResolucion[] = [
  ...
  /**
   * Obtiene y escucha en tiempo real la configuración de la matriz de resolución (3×3) de un área.
   */
  obtenerMatrizPorArea(idArea: string, nombreArea: string = ''): Observable<MatrizResolucion> {
    const areaKey = String(idArea || '1');
    if (this.cacheObservables.has(areaKey)) {
      return this.cacheObservables.get(areaKey)!;
    }

    const obs$ = new Observable<MatrizResolucion>((observer) => {
      ...
      const unsubscribe = onSnapshot(
        ...
        (snapshot) => {
          ...
          const data = snapshot.data() as MatrizResolucion;
        ...
        (error) => {
          console.error('Error al escuchar matriz de resolución:', error);
          observer.next(this.obtenerMatrizPredeterminada(areaKey, nombreArea));
        }
      );
  ...
  /**
   * Guarda o actualiza la matriz de resolución de un área en Firestore.
   */
  async guardarMatrizPorArea(matriz: MatrizResolucion): Promise<void> {
    const idAreaLimpio = String(matriz.idArea);
    const documentoRef = doc(this.firestore, `${this.nombreColeccion}/${idAreaLimpio}`);

    const celdasNormalizadas: CeldaMatrizResolucion[] = matriz.celdas.map((c) => {
    ...
    const payload: MatrizResolucion = {
      idArea: idAreaLimpio,
  ...
  /**
   * Encuentra la celda correspondiente para un par (impacto, urgencia) dentro de una matriz de resolución.
   */
  obtenerCelda(matriz: MatrizResolucion | null | undefined, impacto: number, urgencia: number): CeldaMatrizResolucion {
  ...
  obtenerCeldaPorPrioridad(
    matriz: MatrizResolucion | null | undefined,
    prioridad: 'Crítico' | 'Alto' | 'Medio' | 'Bajo'
  ): CeldaMatrizResolucion {
    if (matriz && matriz.celdas && matriz.celdas.length > 0) {
...
export { MatrizResolucionService as MatrizAtencionService };
