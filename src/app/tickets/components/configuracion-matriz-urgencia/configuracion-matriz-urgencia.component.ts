...
@Component({
  ...
  templateUrl: './configuracion-matriz-urgencia.component.html',
  styleUrl: './configuracion-matriz-urgencia.component.scss'
})
export class ConfiguracionMatrizUrgenciaComponent implements OnInit, OnChanges, OnDestroy {
  @Input() idArea: string = '1';
  @Input() nombreArea: string = '';

  @Output() cerrar = new EventEmitter<void>();
  @Output() guardado = new EventEmitter<MatrizUrgencia>();
  ...
  matrizEditable!: MatrizUrgencia;
  celdaSeleccionada: CeldaMatrizUrgencia | null = null;
  ...
  cargarMatrizArea(): void {
    // Inicializar de inmediato con la matriz por defecto para evitar pantallas congeladas
    this.matrizEditable = this.matrizUrgenciaService.obtenerMatrizPredeterminada(
      this.idArea,
      this.nombreArea
    );
    ...
    this.subscripcionMatriz = this.matrizUrgenciaService
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Error al cargar la matriz de urgencia:', err);
          this.cargando = false;
          this.cdr.detectChanges();
        }
      });
  ...
  obtenerCelda(impacto: number, urgencia: number): CeldaMatrizUrgencia {
    if (!this.matrizEditable || !this.matrizEditable.celdas) {
      const def = this.matrizUrgenciaService.obtenerMatrizPredeterminada(this.idArea, this.nombreArea);
    ...
    return celda || this.matrizUrgenciaService.obtenerCelda(this.matrizEditable, impacto, urgencia);
  }
  ...
  restablecerValoresPredeterminados(): void {
    Swal.fire({
      ...
      text: 'Se cargarán los tiempos predeterminados para esta área (2d, 12h, 2h, 4d, 1d, 8h, 5d, 3d, 1.5d).',
      icon: 'warning',
  ...
  async guardarCambios(): Promise<void> {
    ...
    try {
      ...
      await this.matrizUrgenciaService.guardarMatrizPorArea(this.matrizEditable);

      Swal.fire({
        title: '¡Matriz Guardada!',
        text: `Los tiempos SLA para el área "${this.nombreArea}" han sido actualizados en Firebase.`,
        icon: 'success',
        timer: 2000,
      ...
      console.error('Error al guardar la matriz de urgencia:', err);
      Swal.fire({
        title: 'Error al guardar',
        text: err.message || 'No fue posible guardar la matriz de urgencia.',
        icon: 'error'
      });
    } finally {
